...
func Test_TestNet(t *testing.T) {
	...
	out = gammaCli.Run("send-tx ../jsons/transfer-10-user2.json -env testnet42 -signer user3")
	if !strings.Contains(out, `"ExecutionResult": "SUCCESS"`) {
	...
	out = gammaCli.Run("run-query ../jsons/balanceOf-user2.json -env testnet42")
	if !strings.Contains(out, `"Value": "10"`) {
		t.Fatal("value of user1 after transferFrom failed")
	}
