...
func Test(t *testing.T) {
	...
	out := gammaCli.Run("deploy -name ERC20Token12 -code ../token.go -signer user1")
	if !strings.Contains(out, `"ExecutionResult": "SUCCESS"`) {
		t.Fatal("deploy failed")
	}

	//check if the total amount is okay
	out = gammaCli.Run("read -i ../jsons/totalSupply.json")
	...
	out = gammaCli.Run("send-tx -i ../jsons/approve-user2-50.json -signer user1")
	if !strings.Contains(out, `"ExecutionResult": "SUCCESS"`) {
	...
	out = gammaCli.Run("send-tx -i ../jsons/transfer-10-user2.json -signer user3")
	if !strings.Contains(out, `"ExecutionResult": "SUCCESS"`) {
